...
CLUB_GROUP = {
    # 第1グループ
    '豊前RC': 1, '豊前西RC': 1, '苅田RC': 1, '田川RC': 1, '行橋RC': 1, '行橋COSMOSRC': 1, '行橋みやこRC': 1,
    # 第2グループ
    ...
    '福岡東RC': 4, '福岡東令和あけぼのRC': 4, '福岡城南RC': 4, '福岡南RC': 4,
    '福岡南ファミリアRC': 4, '福岡南RAC': 4, '福岡RAC': 4,
    '福岡東南RC': 4, '福岡東南けやきRC': 4,
    '博多イブニングRC': 4, '博多イブニングトワイライトRC': 4,
    '宗像RC': 4, '対馬RC': 4, '対馬ちんぐRC': 4,
    # 第5グループ
    '福岡中央RC': 5, '福岡中央エンジョイRC': 5, '福岡中央RAC': 5,
    '福岡イブニングRC': 5, '福岡城西RC': 5, '福岡城東RC': 5,
    '福岡北RC': 5, '福岡西RC': 5,
    '博多RC': 5, '壱岐RC': 5, '壱岐中央RC': 5, '糸島RC': 5,
    # 第6グループ
    '甘木RC': 6, '久留米RC': 6, '久留米中央RC': 6, '久留米中央みらいRC': 6,
    '久留米東RC': 6, '久留米北RC': 6,
    '小郡RC': 6, '小郡七夕RC': 6, '鳥栖RC': 6, '浮羽RC': 6,
    # 第7グループ
    '筑後RC': 7, '大川RC': 7,
    '大牟田RC': 7, '大牟田北RC': 7, '大牟田南RC': 7,
    '八女RC': 7, '八女グリーンRC': 7, '柳川RC': 7,
    # 第8グループ (ガバナー事務所)
    'ガバナー事務所': 8,
}
...
def normalize_club(raw: str) -> str:
    ...
    # サフィックス処理
    # "○○ロータリー衛星クラブ" → "○○RC" (現状DBに合わせる)
    if s.endswith('ロータリー衛星クラブ'):
        return s[: -len('ロータリー衛星クラブ')] + 'RC'
    # "○○ローターアクトクラブ" → "○○RAC"
...
for k, v in TAG_CANON.items():
    TAG_LOOKUP[k] = v
# 講演者セクションの中の人はタグ "" (なし) として登録。「■マーカー後」と「ヘッダー行」を区別するため空文字を使う。
TAG_LOOKUP['■協議会 講演者（会長、幹事部門）'] = ''
TAG_LOOKUP['■協議会 講演者(会長、幹事部門)'] = ''
